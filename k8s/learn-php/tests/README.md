# Helm Unit Tests — learn-php Chart

This directory contains unit tests for the `learn-php` Helm chart using the `helm-unittest` plugin.
...
```bash
helm unittest -f 'tests/deployment_test.yaml' k8s/learn-php
helm unittest -f 'tests/service_test.yaml' k8s/learn-php
```

- Run in debug mode (verbose rendering):

```bash
helm unittest -d k8s/learn-php
```

- Generate JUnit report for CI:

```bash
helm unittest --output-type JUnit --output-file test-results.xml k8s/learn-php
```
  ...
  - it: should set custom image tag
    ...
    asserts:
      - equal:
          ...
          value: ghcr.io/dxas90/learn-php:v1.0.0
```
...
- name: Run Helm Unit Tests
  run: helm unittest k8s/learn-php --output-type JUnit --output-file test-results.xml
